...
- 菜单栏以人物徽章显示任务状态，以圆弧显示 Codex 7d 剩余额度
...
- 可选接入 Codex、Claude 实时任务：SSH 推送状态变化，合盖暂停，开盖联网后恢复
...
> 当前 Release 是未公证的通用构建，无需自行编译。首次打开若被系统阻止，请在 `系统设置 > 隐私与安全性` 中选择 `仍要打开`。本版使用开发签名，防睡眠和自动重置需在首次开启时允许后台运行；iCloud 功能已移除。

### Windows

Windows 用户可以使用 `Windows/` 目录下的移植版本，它以系统托盘应用运行，保留账户与套餐、全部额度窗口、Token 用量、30 周热力图、Hook 每日统计以及窗口额度估算，防睡眠与多机器用量中心不在移植范围内。构建与使用说明见 [Windows 移植说明](Windows/README.md)
